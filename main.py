...
# RICH DEFAULT DATA WITH REALISTIC MATCH TIMES & SCORES
DEFAULT_DATA = {
    "matches": [
        {
            "id": 1,
...
# ==========================================
# AUTOMATED MATCH & SCORE FETCHING ENGINE
# ==========================================
def fetch_automated_predictions():
        ...
        try:
            ...
            if res.status_code == 200:
                ...
                for event in events:
                    ...
                    # Estimate safe banker odds if h2h not parsed
                    price = 1.15
    ...
    if len(selected_matches) >= 2:
        ...
        save_data(DATA)
        return True, f"Auto-selected {len(selected_matches)} Banker Matches with LiveScores!"
    else:
...
def trigger_fetch(message):
    bot.reply_to(message, "🔍 Scanning global leagues for 99% Banker matches & LiveScores... Please wait.")
    def run_fetch():
...
<head>
    ...
    <style>
        * { box-sizing: border-box; margin: 0; padding: 0; font-family: 'Segoe UI', Roboto, sans-serif; }
        ...
        .btn { display: block; width: 100%; padding: 14px; margin: 8px 0; border-radius: 8px; text-decoration: none; font-weight: bold; font-size: 14px; text-align: center; border: none; cursor: pointer; }
    ...
    <script>
        function openSportyBetWithCode(code, targetUrl) {
            try {
                if (navigator.clipboard) {
                    navigator.clipboard.writeText(code);
                }
            } catch(e) {}
            alert("🔑 Booking Code [" + code + "] Copied!\n\nRedirecting to SportyBet...");
...
<div class="container">
    ...
    <div class="card">
        ...
        <p style="font-size:11px; color:#94a3b8; margin-bottom:10px;">💡 <i>Tap any match below to view LiveScore Center!</i></p>
        ...
        <!-- UNBLOCKABLE MOBILE REDIRECT BUTTON -->
        <button onclick="openSportyBetWithCode('{{ data.get('sportybet_code', 'BC982A1') }}', '{{ data.get('affiliate_link', 'https://www.sportybet.com/ng/') }}')" 
...
# MATCH DETAILS & REAL LIVESCORE SUBPAGE TEMPLATE
MATCH_HTML = """
<!DOCTYPE html>
<html lang="en">
<head>
    ...
    <style>
        * { box-sizing: border-box; margin: 0; padding: 0; font-family: 'Segoe UI', Roboto, sans-serif; }
        ...
        .live-box { background: #0f172a; padding: 20px; border-radius: 10px; border: 1px solid #0284c7; text-align: center; margin-top: 15px; }
        .live-title { color: #38bdf8; font-size: 14px; font-weight: bold; margin-bottom: 8px; }
        .score { font-size: 28px; font-weight: bold; color: #22c55e; letter-spacing: 3px; margin: 10px 0; }
        .status { font-size: 12px; color: #fbbf24; font-weight: bold; }
        .btn { display: block; width: 100%; padding: 14px; margin-top: 15px; border-radius: 8px; text-decoration: none; font-weight: bold; font-size: 14px; text-align: center; background: #e11d48; color: white; border: none; cursor: pointer; }
    </style>
</head>
<body>
<div class="container">
    <div style="text-align:left;">
        <a href="/" class="back-btn">⬅ Back to Ticket</a>
    </div>
    ...
    <div class="card">
        ...
        <div class="live-box">
            <div class="live-title">⚡ REAL-TIME MATCH LIVESCORE</div>
            <div class="score">{{ match.get('score', 'VS') }}</div>
            <div class="status">Status: {{ match.get('status', 'Upcoming') }}</div>
        </div>
